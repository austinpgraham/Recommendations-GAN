...
class GAN():
    ...
    def __init__(self, d, d_optimizer, d_net, d_loss, g, cond, g_optimizer, g_net, g_loss, keep_prob):
...
def gan(dis_arch, gen_arch, conditional):
    ...
    d_opt = tf.train.AdamOptimizer(0.01).minimize(d_total_loss, var_list=d.get_var_list())
    g_opt = tf.train.AdamOptimizer(0.01).minimize(g_loss, var_list=g.get_var_list())
    return GAN(X, d_opt, d, d_total_loss, Z, Y, g_opt, g, g_loss, keep_prob)
