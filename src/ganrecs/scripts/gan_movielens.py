...
def write_output(d_losses, g_losses, distances):
    ...
    json.dump(_dict, open('out/out50000epochsnosvd.json', 'w'))
...
def main(args=None):
    ...
    if os.path.exists(model_path + ".meta"):
        ...
        for i in range(len(rc.folds)):
            ...
            network = gan(dis_arch, gen_arch, MOVIES_COUNT, 50)
            session = tf.Session()
            ...
            for it in range(epochs):
                ...
                network.generator_condition: users, network.pert: users_p})
